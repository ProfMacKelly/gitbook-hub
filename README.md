---
icon: house
---

# Home

## Welcome to Studies De Jure

The primary purpose of this site will be to host [open educational resources](#user-content-fn-1)[^1] for undergraduate legal course content. "Open license" refers to a license (in essence, permission) to&#x20;

{% hint style="warning" %}
:construction\_site: _This site is a work in progress_, so most of the content is not in the "final" form. :construction:
{% endhint %}

These resources are [licensed openly](#user-content-fn-2)[^2], free to be reused, redistributed, [adapted, and/or remixed](https://help.oercommons.org/support/solutions/articles/42000046860-remix-a-resource), and include content that was authored by Matthew L. Mac Kelly and/or adapted, remixed, and/or incorporated from other creators in compliance with copyright and licensing regulations and practices. Attributions and licensing information is identified on each page.

\~ _Matt Mac Kelly, JD, EdD_

[^1]: Free educational material that is openly licensed to enable reuse and redistribution by users ([Elder, 2019](https://iastate.pressbooks.pub/oerstarterkit/chapter/introduction/)).

[^2]: See the Front Matter for more information.
